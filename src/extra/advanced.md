...
It's always best to setup the simplest setup possible, introducing complexity only as necessary
...
The most important aspect of Invariant Testing is what you actually test
...
Mocking is a destructive operation, it casues a loss of information

But it's also a simplification of the real system, leading the tools to be a lot faster

Overall you should be mindful of how State is explored
...
Your handles can infuence who receives donations
...
Programmatic Deployments by definiton make the fuzzer a lot slower, however they turn simple suites into multi-dimensional ones
...
Roughly speaking programmatic deployment consits of adding 4 functions:
- A `deployNew` handler, which will deploy a new version of the target
- A `switchTo` handler, to change the current target being used (which allows to write simpler tests while having multi-dimensionality)
...
We're pretty confident in this patten and believe you should be rolling your own Managers whenever you need to add multi-dimensionality
...
This is a fairly underdiscussed issue with all forms of testing and formal verification

You can only test states that are feasible given your setup

A contracte fuzzer has a statistical likelihood of hitting all states

A symbolic fuzzer should guarantee that they will hit those states, however it may never terminate

Anyhow, you should map out what behaviour is possible and what's not possible

Based on this you will be able to write properties

These properties won't be necessarily "true", they will be true for the system you setup
...
For example you may have a bunch of deposit / mint operations which "add" and others that "remove"
...
This will allow you write very elegant properties
...
Because of how revert and assertions are handled this still count for the coverage and stil triggers assertion failures
