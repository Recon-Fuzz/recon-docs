...
Implementing properties is the most important part of invariant testing, here we'll look at the [different types](#property-types) of properties that you can define and [how these can be implemented](#inlined-vs-global-properties) (inlined or global) along with the different techniques that you can use to implement your properties as code using an ERC4626 vault as an [example](#example).

## What are properties? 
**Properties** allow us to define behaviors that we expect in our system. In the context of testing we can say that properties are logical statements about the system that should hold true after state-changing operations are made via a call to one of the target function [handlers](../using_recon/building_handlers.md#what-are-handlers). 
...
> We prefer to use the term properties throughout this book because it allows us to include invariants as well properties since invariants are a subset of properties but properties are not a subset of invariants.
...
At Recon we've adopted the practice of exclusively using assertion mode because it simplifies the test writing process and allows us to run only one job with Echidna in assertion mode rather than one job in assertion and one in property mode because Echidna doesn't support using two testing modes at the same time.
